...
public class Memory {
    ...
    // 主存大小 32 MB
    public static int PAGE_SIZE_B = 1024;
    // 页大小 1 KB
    ...
    private char[] readForManage(String eip, int len) {
        char[] data = new char[len];
        int from = Integer.parseInt(new Transformer().binaryToInt(eip));
        System.arraycopy(memory, from, data, 0, len);
        return data;
    }
     ...
     * @param len 数据段长度
     */
    public void realLoad(String eip, int len) {
     ...
     * @return 该段的内存基址
     */
    public String segLoad(int segIndex) {
        SegDescriptor targetSeg = segTbl.get(segIndex);

        char[] limit = targetSeg.getLimit();
        // 20位偏移
        char[] diskBase = targetSeg.getDisk();
        // 32位磁盘地址
        ...
        if (Memory.PAGE) {
            ...
            char[] data = Disk.getDisk().read(String.valueOf(diskBase), len);
     ...
     */
    public String pageLoad(int segIndex, int pageNO) {
        ...
        // 段内第一个物理页和最后一个物理页
        int startFrame = base / PAGE_SIZE_B;
        int endFrame = (base + limit) / PAGE_SIZE_B - 1;
    ...
    /**
     * 最先适应算法
     * 在进行fit的时候，已经确保了段内有足够的空间来放下这个segment
     *
    ...
    public void fit(int index, char[] data, int len) {
        List<AbstractMap.SimpleEntry<Integer, Integer>> occupiedSegs = new ArrayList<>();
        // 存储每个被占用的内存空间的segDescriptor的base与index

        for (SegDescriptor segDes : segTbl) {
            if (segDes.isValidBit()) {
                // 已经被占用
        ...
        // 按起始位置从小到大排序
        occupiedSegs.sort(Comparator.comparingInt(AbstractMap.SimpleEntry::getKey));
        ...
        for (AbstractMap.SimpleEntry<Integer, Integer> occupied : occupiedSegs) {
            ...
            if (occupied.getKey() != current) {
                // 整理碎片，把下一个segment的数据拷贝到前一段
                char[] segData = readForManage(String.valueOf(segDes.getBase()), limit);
    ...
    private static class SegDescriptor {
        ...
        // 31位限长，表示段在内存中的长度
        private char[] limit = new char[31];
        // 有效位,为true表示被占用（段已在内存中），为false表示空闲（不在内存中）
        private boolean validBit = false;

        private long timeStamp;

        // 段在物理磁盘中的存储位置，真实段描述符里不包含此字段，本作业规定，段在磁盘中连续存储，并且磁盘中的存储位置不会发生变化
        private char[] diskBase = new char[32];
     ...
     */
    private static class ReversedPageItem {
